...
static int nadekoColumn(sqlite3_vtab_cursor *pVtabCur, /* The cursor */
    ...
    case NADEKO_FILENAME: {
        char *zSql = sqlite3_mprintf("SELECT filename FROM %s.%s WHERE rowid == ?",
            pCur->pParent->zDb,
            pCur->pParent->zTable);
        sqlite3_stmt *pStmt;
    ...
    default:
        ...
        sqlite3_blob_open(pCur->pParent->db,
            pCur->pParent->zDb,
            pCur->pParent->zTable,
            "contents",
            pCur->iRowid,
            0,
            &pBlob);
        sqlite3_result_blob(pCtx, pBlob, sqlite3_blob_bytes(pBlob), 0);
